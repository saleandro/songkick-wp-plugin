...
class SongkickGigography {
	public $apikey;
	public $upcoming_gigs = array();

	function SongkickGigography($apikey) {
		$this->apikey = trim($apikey);
		$this->apiurl = 'http://api.songkick.com/api/3.0';
	}

	function get_upcoming_gigs($per_page=10) {
		$cached_results = $this->get_cached_results($this->url($per_page));
		if ($this->cache_expired($cached_results)) {
			$gigs = $this->get_uncached_upcoming_gigs($per_page);
			$cached_results = array('gigs' => $gigs, 'timestamp'=> time());
			$this->set_cached_results($this->url($per_page), $cached_results);
		} else {
			$gigs = $cached_results['gigs'];
		}
		return $gigs;
	}
	...
	protected function get_uncached_upcoming_gigs($per_page) {
		$response = $this->fetch($this->url($per_page));
		return $this->events_from_json($response);
	}
